...
int main(int argc, char **argv)
{
    ...
    q_f << 0, 0.0, 40.0, 60.0, 30.0, 0.0, 90.0;
    ...
    q_init << 0, 0, 0, 0, 0, 0, 0;   // in deg
    q_init = q_init * (180 / M_PI);
    ...
    atomic<bool> is_position_control_running(true);
    ...
    std::thread position_thread([&]() {
            std::cout << base_frame << "\n";
            joint_control_execution(base,base_cyclic, actuator_config, base_feedback, base_command, robot, joint_trajectory, base_frame, CONTROL_FREQUENCY,  std::ref(is_position_control_running));
        });
